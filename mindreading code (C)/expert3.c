...
char *argv[];
{
  ...
  num_trials = 201;             /* set default values */
  ftime(&tp);
  random_seed = tp.time * tp.millitm;
  encryption_ON = 0;
  archive_file_name = ALLOC(char, 128);
  sprintf(archive_file_name, "%s/%s", ARCHIVE_DIR, getulogin());

  for (i = 1, error = 0; i < argc && !error; i++) {
    if (strcmp(argv[i], "-l") == 0)  /* set num_trials */
      error = (++i >= argc || !strtoint(argv[i], &num_trials));
    else if (strcmp(argv[i], "-r") == 0)  /* set random seed */
	...
	archive_file_name = argv[i];
      ...
      fprintf(tty, "  -l  <n>    : set number of trials to <n> (0 for unlimited trials)\n");
...
char *argv[];
{
  fprintf(tty, "Think of a bit (0 or 1), and I'll try to guess what bit you're thinking of.\n");
  fprintf(tty, "After you type your bit, I'll tell you what my guess was.\n");
  fprintf(tty, "If my guess was correct, I win the round.  Otherwise you win the round.\n");
  fprintf(tty, "After each round, I'll show who's leading, and by how much:\n");
  fprintf(tty, "   '-' means I'm ahead, '+' means you're ahead.\n");
  fprintf(tty, "I'll also show the percentage of the rounds that I won so far.\n");

  if (fixed_len_mode)
    fprintf(tty, "We'll play for %d rounds.\n",num_trials);
  else
    fprintf(tty, "We'll play for as many rounds as you like (^C to quit).\n");
  fprintf(tty, "Type '%s -h' to see what options are available\n\n",argv[0]);
}
...
{
  fprintf(tty, "\n");
  fprintf(tty, "Final score: I won %d rounds, and you won %d rounds.\n",
	  num_wins, num_trials-num_wins);
  if(num_wins > num_trials - num_wins)
    fprintf(tty, 
    "The probability that I would do so well by random guessing is: %.3lg\n",
	    binomial_tail(num_trials, num_trials-num_wins));
  else if(num_wins < num_trials - num_wins)
    fprintf(tty, 
    "The probability that you would do so well by random guessing is: %.3lg\n",
	    binomial_tail(num_trials, num_wins));
}
...
char *argv[];
{
  ...
  loss = 0.0;
  initialize_archive();
  for(trial=1;!fixed_len_mode || trial<=num_trials;trial++) {
    if (!fixed_len_mode)
      compute_beta();
    p = get_experts_pred();
    ...
    t = trial - 2*num_wins;
    if (abs(t) <= SCOREWIDTH) {
      for(j = -SCOREWIDTH; j < 0; j++)
	fprintf(tty, "%c", (j < t ? ' ' : '-'));
      fprintf(tty, "|");
      for(j = 1; j <= SCOREWIDTH; j++)
	fprintf(tty, "%c", (j > t ? ' ' : '+'));
    }
    else if (t > SCOREWIDTH) {
      for (j = -SCOREWIDTH; j<0; j++) fprintf(tty, " ");
      fprintf(tty, "|");
      for (j = 1; j <= SCOREWIDTH-SCOREDIGITS-2; j++) fprintf(tty, "+");
      fprintf(tty, "(%*d)",SCOREDIGITS,abs(t));
    }
    else if (t < SCOREWIDTH) {
      fprintf(tty, "(%*d)",SCOREDIGITS,abs(t));
      for (j = -SCOREWIDTH+SCOREDIGITS+2; j<0; j++) fprintf(tty, "-");
      fprintf(tty, "|");
      for (j = 1; j <= SCOREWIDTH; j++) fprintf(tty, " ");
    }
    fprintf(tty, " (%.1lf%%)", 100.*(trial-num_wins)/trial);
  }
  if (input_from_tty) system("stty -raw echo");
  print_final_comments();
  archive_weights();
