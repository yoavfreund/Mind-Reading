...
public class mindreader extends PApplet {
  ...
  void drawplayers() {
    ...
    cx = (int)(300+285*cos(comwins*TWO_PI/100.00)); //was 50 for 2 laps
    cy = (int)(200+165*sin(comwins*TWO_PI/100.00));
    hx = (int)(300+285*cos(humwins*TWO_PI/100.00));
    hy = (int)(200+165*sin(humwins*TWO_PI/100.00));  
    noStroke();
    fill(200,0,0);
    ellipse(cx,cy,20,20);
    fill(0,0,200);
  ...
  void restartWriteFile()
 {
  ...
  void drawsequence() {
    ...
    for(int i=0; i<curgameindex && i<=67; i++) {
      preds += str(sequence[i][0])+"";
//
	System.out.println("SEQUENCE = " + sequence[i][0]);
  ...
  void drawstats() {
    ...
    for(int i=0; i<10; i++) {
      ...
      text("TEST"+stats[i], 180, 118+(i+1)*17);
    } 

    textFont(font2, 24);
    text("High % of Wins", 310, 100);
    textFont(font2, 14);
  ...
  void keyPressed() 
  {
    if (initsetup == 0) {
      ...
      case '1': 
        ...
        break;
      case 'm':
        current = (generator.nextDouble()  < .5 ? 0 : 1);
        cheating = true;
        process();
        break;
      case 'r':
        //sendData(); Call to sendData to output info to file
